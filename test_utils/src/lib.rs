...
impl TestContext {
    ...
    async fn apply_operator_manifests(&self) -> anyhow::Result<()> {
        ...
        // Apply CRDs from temp directory
        ...
        // Copy and modify RBAC files
        test_info!(&self.test_name, "Preparing RBAC manifests");

        // Copy service account and update namespace
        ...
        // Read generated role.yaml and add namespace prefix
        ...
        // Read and modify role_binding to use prefixed ClusterRole name
        ...
        // Copy and modify leader election files
...
"#
        ...
        // Update CR manifest with publicTrusteeAddr using yq
        test_info!(
            &self.test_name,
        ...
        let yq_output = Command::new("yq")
            .args([
                ".spec.publicTrusteeAddr = \"".to_string() + &trustee_addr + "\"",
                "-i".to_string(),
                cr_manifest_str.to_string(),
            ])
            .output()
            .await?;

        if !yq_output.status.success() {
            let stderr = String::from_utf8_lossy(&yq_output.stderr);
            return Err(anyhow::anyhow!("Failed to update CR with yq: {stderr}"));
        }
